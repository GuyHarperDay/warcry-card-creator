...
drawWeapon = function (weaponData, pixelPosition) {
  ...
  writeScaled(weaponData.attacks, { x: pixelPosition.x + 270, y: statsPosY });

  writeScaled(weaponData.strength, { x: pixelPosition.x + 365, y: statsPosY });

  writeScaled(weaponData.damageBase + '/' + weaponData.damageCrit, { x: pixelPosition.x + 460, y: statsPosY });
...
function setSelectedRunemark(radioDiv, runemark, radioGroupName, bgColor) {
  ...
  if (runemark != null) {
    ...
    if (img.length > 0) {
      ...
      var newDiv = addToImageRadioSelector(runemark, radioDiv, radioGroupName, bgColor);
...
function writeWeaponControls(weaponId, weaponData, weaponName) {
  ...
  setSelectedRunemark(weaponDiv.find('#weaponRunemarkSelect')[0], weaponData.runemark, weaponName, 'white');
}
...
render = function (fighterData) {
  ...
  getContext().font = '50px frzquadb';
  getContext().fillStyle = '#ECECEC';
...
async function saveFighterData(fighterData) {
  ...
  if (fighterData != null && fighterData.name) {
...
function refreshSaveSlots() {
  ...
  for (let [key, value] of Object.entries(map)) {
    var selected = false;
    if (fighterDataName && key == fighterDataName) {
      selected = true;
    }
